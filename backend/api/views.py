...
def duplicate_entry(request, pk):
    ...
    new_entry.save()
    return Response(DutySlipEntrySerializer(new_entry).data, status=status.HTTP_201_CREATED)
...
def push_backup_github(request):
    ...
    return Response({'error': 'Failed to push to GitHub. Check your token and repo name.'}, status=500)
